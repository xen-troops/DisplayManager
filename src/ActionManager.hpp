...
enum class EventType { CREATE, DESTROY, USER };

enum class ObjectType { LAYER, SURFACE };
...
public:
    ActionSource(ObjectManager& objects, ObjectType type,
        ...
        : ActionObject("SOURCE", objects, type, name), mSource(source)
    {
    }

    void perform() override { getObject()->setSource(mSource); }
...
public:
    ActionDestination(ObjectManager& objects, ObjectType type,
                      const std::string& name, const IlmRectangle& destination)
        : ActionObject("DESTINATION", objects, type, name),
          mDestination(destination)
    {
    }

    void perform() override { getObject()->setDestination(mDestination); }
...
public:
    ActionParent(ObjectManager& objects, ObjectType type,
        ...
        : ActionObject("PARENT", objects, type, name), mParentName(parent)
    {
    }
...
public:
    ActionOrder(ObjectManager& objects, ObjectType type,
        ...
        : ActionObject("ORDER", objects, type, name), mOrder(order)
    {
    }

    void perform() override { getObject()->setOrder(mOrder); }
...
public:
    ActionVisibility(ObjectManager& objects, ObjectType type,
        ...
        : ActionObject("VISIBILITY", objects, type, name), mValue(value)
    {
    }

    void perform() override { getObject()->setVisibility(mValue); }
...
public:
    ActionOpacity(ObjectManager& objects, ObjectType type,
        ...
        : ActionObject("OPACITY", objects, type, name), mValue(value)
    {
    }

    void perform() override { getObject()->setOpacity(mValue); }
...
class Event {
public:
    Event(EventType event) : mEvent(event), mLog("Event") {}
...
public:
    EventObject(EventType event, ObjectType object, const std::string& name)
        : Event(event), mObject(object), mName(name)
    {
    }
...
public:
    EventUser(uint32_t id) : Event(EventType::USER), mID(id) {}
