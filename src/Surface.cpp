...
void Surface::setVisibility(t_ilm_bool visibility)
{
    ...
    if (ret != ILM_SUCCESS) {
        throw DmException("Can't set surface visibility: " + to_string(mID),
                          ret);
    }
...
void Surface::setSource(const IlmRectangle& rect)
{
    ...
    auto ret = ilm_surfaceSetSourceRectangle(mID, rect.x, rect.y, rect.width,
                                             rect.height);

    if (ret != ILM_SUCCESS) {
        throw DmException("Can't set surface source: " + to_string(mID), ret);
    }
...
void Surface::setDestination(const IlmRectangle& rect)
{
    ...
    if (ret != ILM_SUCCESS) {
        throw DmException("Can't set surface destination: " + to_string(mID),
                          ret);
    }
...
void Surface::onOpacity(t_ilm_float opacity)
{
    if (opacity != mOpacity) {
        ...
        if (ret != ILM_SUCCESS) {
            throw DmException("Can't set surface opacity: " + to_string(mID),
                              ret);
        }
...
void Surface::onSource(const IlmRectangle& rect)
{
    if (mSource != rect) {
        ...
        if (ret != ILM_SUCCESS) {
            throw DmException("Can't set surface source: " + to_string(mID),
                              ret);
        }
...
void Surface::onDestination(const IlmRectangle& rect)
{
    if (mDestination != rect) {
        ...
        auto ret = ilm_surfaceSetDestinationRectangle(
            mID, mDestination.x, mDestination.y, mDestination.width,
            mDestination.height);

        if (ret != ILM_SUCCESS) {
            throw DmException(
                "Can't set surface destination: " + to_string(mID), ret);
        }
...
void Surface::getProperties()
{
    ...
    if (ret != ILM_SUCCESS) {
        throw DmException("Can't get surface properties: " + to_string(mID),
                          ret);
    }
    ...
    mSource = {property.sourceX, property.sourceY, property.sourceWidth,
               property.sourceHeight};

    mDestination = {property.destX, property.destY, property.destWidth,
                    property.destHeight};
